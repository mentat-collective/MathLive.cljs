...
depSel=!0:0==((null!==(_a=addresses[dep.id])&&void 0!==_a?_a:1)&1)&&depAddrs.push(addresses[dep.id]);return{create(state){state.values[idx]=getter(state);return 1},update(state,tr){if(depDoc&&tr.docChanged||depSel&&(tr.docChanged||tr.selection)||ensureAll(state,depAddrs))if(tr=getter(state),multi?!compareArray(tr,state.values[idx],compare):!compare(tr,state.values[idx]))return state.values[idx]=tr,1;return 0},reconfigure:(state,oldState)=>{let newVal=getter(state);var oldAddr=oldState.config.address[id];
if(null!=oldAddr&&(oldAddr=getAddr(oldState,oldAddr),this.dependencies.every(dep=>dep instanceof Facet?oldState.facet(dep)===state.facet(dep):dep instanceof StateField?oldState.field(dep,!1)==state.field(dep,!1):!0)||(multi?compareArray(newVal,oldAddr,compare):compare(newVal,oldAddr))))return state.values[idx]=oldAddr,0;state.values[idx]=newVal;return 1}}}}const initField=Facet.define({static:!0});class StateField{constructor(id,createF,updateF,compareF,spec){this.id=id;this.createF=createF;this.updateF=
updateF;this.compareF=compareF;this.spec=spec;this.provides=void 0}static define(config){let field=new StateField(nextID++,config.create,config.update,config.compare||((a,b)=>a===b),config);config.provide&&(field.provides=config.provide(field));return field}create(state){let init=state.facet(initField).find(i=>i.field==this);return((null===init||void 0===init?void 0:init.create)||this.createF)(state)}slot(addresses){let idx=addresses[this.id]>>1;return{create:state=>{state.values[idx]=this.create(state);
return 1},update:(state,tr)=>{let oldVal=state.values[idx];tr=this.updateF(oldVal,tr);if(this.compareF(oldVal,tr))return 0;state.values[idx]=tr;return 1},reconfigure:(state,oldState)=>{if(null!=oldState.config.address[this.id])return state.values[idx]=oldState.field(this),0;state.values[idx]=this.create(state);return 1}}}init(create){return[this,initField.of({field:this,create})]}get extension(){return this}}const Prec_={lowest:4,low:3,default:2,high:1,highest:0};global={highest:prec(Prec_.highest),
high:prec(Prec_.high),default:prec(Prec_.default),low:prec(Prec_.low),lowest:prec(Prec_.lowest)};class PrecExtension{constructor(inner,prec){this.inner=inner;this.prec=prec}}class Compartment{of(ext){return new CompartmentInstance(this,ext)}reconfigure(content){return Compartment.reconfigure.of({compartment:this,extension:content})}get(state){return state.config.compartments.get(this)}}class CompartmentInstance{constructor(compartment,inner){this.compartment=compartment;this.inner=inner}}class Configuration{constructor(base,
...
newCompartments=new Map;for(var ext of flatten(base,compartments,newCompartments))ext instanceof StateField?fields.push(ext):(facets[ext.facet.id]||(facets[ext.facet.id]=[])).push(ext);let address=Object.create(null);compartments=[];ext=[];for(let field of fields)address[field.id]=ext.length<<1,ext.push(a=>field.slot(a));fields=null===oldState||void 0===oldState?void 0:oldState.config.facets;for(let id in facets){let providers=facets[id],facet=providers[0].facet;var oldProviders=fields&&fields[id]||
[];if(providers.every(p=>0==p.type))address[facet.id]=compartments.length<<1|1,sameArray(oldProviders,providers)?compartments.push(oldState.facet(facet)):(oldProviders=facet.combine(providers.map(p=>p.value)),compartments.push(oldState&&facet.compare(oldProviders,oldState.facet(facet))?oldState.facet(facet):oldProviders));else{for(let p of providers)0==p.type?(address[p.id]=compartments.length<<1|1,compartments.push(p.value)):(address[p.id]=ext.length<<1,ext.push(a=>p.dynamicSlot(a)));address[facet.id]=
ext.length<<1;ext.push(a=>dynamicFacetSlot(a,facet,providers))}}oldState=ext.map(f=>f(address));return new Configuration(base,newCompartments,oldState,address,compartments,facets)}}const languageData=Facet.define(),allowMultipleSelections=Facet.define({combine:values=>values.some(v=>v),static:!0}),lineSeparator=Facet.define({combine:values=>values.length?values[0]:void 0,static:!0}),changeFilter=Facet.define(),transactionFilter=Facet.define(),transactionExtender=Facet.define(),readOnly=Facet.define({combine:values=>
values.length?values[0]:!1});class Annotation{constructor(type,value){this.type=type;this.value=value}static define(){return new AnnotationType}}class AnnotationType{of(value){return new Annotation(this,value)}}class StateEffectType{constructor(map){this.map=map}of(value){return new StateEffect(this,value)}}class StateEffect{constructor(type,value){this.type=type;this.value=value}map(mapping){mapping=this.type.map(this.value,mapping);return void 0===mapping?void 0:mapping==this.value?this:new StateEffect(this.type,
mapping)}is(type){return this.type==type}static define(spec={}){return new StateEffectType(spec.map||(v=>v))}static mapEffects(effects,mapping){if(!effects.length)return effects;let result=[];for(let effect of effects)(effects=effect.map(mapping))&&result.push(effects);return result}}StateEffect.reconfigure=StateEffect.define();StateEffect.appendConfig=StateEffect.define();class Transaction{constructor(startState,changes,selection,effects,annotations,scrollIntoView){this.startState=startState;this.changes=
...
(this._doc=this.changes.apply(this.startState.doc))}get newSelection(){return this.selection||this.startState.selection.map(this.changes)}get state(){this._state||this.startState.applyTransaction(this);return this._state}annotation(type){for(let ann of this.annotations)if(ann.type==type)return ann.value}get docChanged(){return!this.changes.empty}get reconfigured(){return this.startState.config!=this.state.config}isUserEvent(event){let e=this.annotation(Transaction.userEvent);return!(!e||!(e==event||
e.length>event.length&&e.slice(0,event.length)==event&&"."==e[event.length]))}}Transaction.time=Annotation.define();Transaction.userEvent=Annotation.define();Transaction.addToHistory=Annotation.define();Transaction.remote=Annotation.define();const none=[];exports.CharCategory=void 0;(function(CharCategory){CharCategory[CharCategory.Word=0]="Word";CharCategory[CharCategory.Space=1]="Space";CharCategory[CharCategory.Other=2]="Other"})(exports.CharCategory||(exports.CharCategory={}));const nonASCIISingleCaseWordChar=
/[\u00df\u0587\u0590-\u05f4\u0600-\u06ff\u3040-\u309f\u30a0-\u30ff\u3400-\u4db5\u4e00-\u9fcc\uac00-\ud7af]/;let wordChar;try{wordChar=RegExp("[\\p{Alphabetic}\\p{Number}_]","u")}catch(_){}class EditorState{constructor(config,doc,selection,values,computeSlot,tr){this.config=config;this.doc=doc;this.selection=selection;this.values=values;this.status=config.statusTemplate.slice();this.computeSlot=computeSlot;tr&&(tr._state=this);for(config=0;config<this.config.dynamicSlots.length;config++)ensureAddr(this,
config<<1);this.computeSlot=null}field(field,require=!0){field=this.config.address[field.id];if(null==field){if(require)throw new RangeError("Field is not present in this state");}else return ensureAddr(this,field),getAddr(this,field)}update(...specs){return resolveTransaction(this,specs,!0)}applyTransaction(tr){let conf=this.config,{base,compartments}=conf;for(var effect of tr.effects)effect.is(Compartment.reconfigure)?(conf&&(compartments=new Map,conf.compartments.forEach((val,key)=>compartments.set(key,
val)),conf=null),compartments.set(effect.value.compartment,effect.value.extension)):effect.is(StateEffect.reconfigure)?(conf=null,base=effect.value):effect.is(StateEffect.appendConfig)&&(conf=null,base=asArray(base).concat(effect.value));conf?effect=tr.startState.values.slice():(conf=Configuration.resolve(base,compartments,this),effect=(new EditorState(conf,this.doc,this.selection,conf.dynamicSlots.map(()=>null),(state,slot)=>slot.reconfigure(state,this),null)).values);new EditorState(conf,tr.newDoc,
tr.newSelection,effect,(state,slot)=>slot.update(state,tr),tr)}replaceSelection(text){"string"==typeof text&&(text=this.toText(text));return this.changeByRange(range=>({changes:{from:range.from,to:range.to,insert:text},range:EditorSelection.cursor(range.from+text.length)}))}changeByRange(f){let sel=this.selection;var result1=f(sel.ranges[0]);let changes=this.changes(result1.changes),ranges=[result1.range];result1=asArray(result1.effects);for(let i=1;i<sel.ranges.length;i++){let result=f(sel.ranges[i]);
var newChanges=this.changes(result.changes);let newMapped=newChanges.map(changes);for(let j=0;j<i;j++)ranges[j]=ranges[j].map(newMapped);newChanges=changes.mapDesc(newChanges,!0);ranges.push(result.range.map(newChanges));changes=changes.compose(newMapped);result1=StateEffect.mapEffects(result1,newMapped).concat(StateEffect.mapEffects(asArray(result.effects),newChanges))}return{changes,selection:EditorSelection.create(ranges,sel.mainIndex),effects:result1}}changes(spec=[]){return spec instanceof ChangeSet?
spec:ChangeSet.of(spec,this.doc.length,this.facet(EditorState.lineSeparator))}toText(string){return Text.of(string.split(this.facet(EditorState.lineSeparator)||DefaultSplit))}sliceDoc(from=0,to=this.doc.length){return this.doc.sliceString(from,to,this.lineBreak)}facet(facet){let addr=this.config.address[facet.id];if(null==addr)return facet.default;ensureAddr(this,addr);return getAddr(this,addr)}toJSON(fields){let result={doc:this.sliceDoc(),selection:this.selection.toJSON()};if(fields)for(let prop in fields){let value=
fields[prop];value instanceof StateField&&null!=this.config.address[value.id]&&(result[prop]=value.spec.toJSON(this.field(fields[prop]),this))}return result}static fromJSON(json,config={},fields){if(!json||"string"!=typeof json.doc)throw new RangeError("Invalid JSON representation for EditorState");let fieldInit=[];if(fields)for(let prop in fields)if(Object.prototype.hasOwnProperty.call(json,prop)){let field=fields[prop],value=json[prop];fieldInit.push(field.init(state=>field.spec.fromJSON(value,
state)))}return EditorState.create({doc:json.doc,selection:EditorSelection.fromJSON(json.selection),extensions:config.extensions?fieldInit.concat([config.extensions]):fieldInit})}static create(config={}){let configuration=Configuration.resolve(config.extensions||[],new Map),doc=config.doc instanceof Text?config.doc:Text.of((config.doc||"").split(configuration.staticFacet(EditorState.lineSeparator)||DefaultSplit));config=config.selection?config.selection instanceof EditorSelection?config.selection:
EditorSelection.single(config.selection.anchor,config.selection.head):EditorSelection.single(0);checkSelection(config,doc.length);configuration.staticFacet(allowMultipleSelections)||(config=config.asSingle());return new EditorState(configuration,doc,config,configuration.dynamicSlots.map(()=>null),(state,slot)=>slot.create(state),null)}get tabSize(){return this.facet(EditorState.tabSize)}get lineBreak(){return this.facet(EditorState.lineSeparator)||"\n"}get readOnly(){return this.facet(readOnly)}phrase(phrase,
